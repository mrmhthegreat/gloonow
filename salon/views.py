...
def index(request):
    ...
    p=BookingPost.objects.search().order_by('-bookdatetime')[:10]
    form=FilterBook()
...
class PostListView( FormMixin,ListView):
    ...
    def get_queryset(self):
        ...
        queryset=BookingPost.objects.search().order_by('-bookdatetime')
        if self.request.POST.get("services")or self.request.POST.get("dates"):
            ...
            if form.is_valid():
                ...
                if(date and selection ):
                    ...
                    queryset=BookingPost.objects.filterspecfice(ct,selection).order_by('-bookdatetime')
                elif(selection):
                    queryset = BookingPost.objects.filterspecficesv(selection).order_by('-bookdatetime')
                elif(date):
                    ct=datetime.strptime(date.strip(),'%d %b %Y')

                    queryset = BookingPost.objects.filterdatespecficesv(ct).order_by('-bookdatetime')
